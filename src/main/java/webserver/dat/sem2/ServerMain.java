...
//        System.out.println( getFile("adding.html") );
    ...
    private static void picoServer05grøn() throws Exception {
        final ServerSocket server = new ServerSocket(8080);
        System.out.println("Listening for connection on port 8080 ....");
        String root = "pages";
    ...
    private static void serverTask(Socket socket, String root) throws IOException {
        try {
            System.out.println("-----------------");
            HttpRequest req = new HttpRequest(socket.getInputStream());
            String path = root + req.getPath();
...
//        System.out.println( getFile("adding.html") );
    ...
    private static void picoServer06gul() throws Exception {
        final ServerSocket server = new ServerSocket(8080);
        System.out.println("Listening for connection on port 8080 ....");
        String root = "pages";
        int count = 0;
            ...
            try {
                ...
                if (path.endsWith(".html") || path.endsWith(".txt")) {
                    workingJack.submit(new Runnable() {
                        ...
                        public void run() {
                            try {
                                returnStaticFile(root, path, socket);
                                socketClose(socket);
                            } catch (Exception ex) {
                    ...
                    workingJack.submit(new Runnable() {
                        ...
                        public void run() {
                            try {
                                addNumbersTask(path, req, socket);
                                socketClose(socket);
...
//        System.out.println( getFile("adding.html") );
    ...
    private static void picoServer06rød() throws Exception {
        final ServerSocket server = new ServerSocket(8080);
        System.out.println("Listening for connection on port 8080 ....");
        String root = "pages";
        int count = 0;
            ...
            workingJack.submit(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        HttpRequest req = new HttpRequest(socket.getInputStream());
                        String path = req.getPath();
                        tasks(path, workingJack, root, socket, req);
                    } catch (IOException ex) {
    ...
    private static void tasks(String path, ExecutorService workingJack, String root, Socket socket, HttpRequest req) {
        if (path.endsWith(".html") || path.endsWith(".txt")) {
            workingJack.submit(new Runnable() {
                @Override
                public void run() {
    ...
    private static void socketClose(Socket socket) throws IOException {
        if (socket != null) {
            socket.close();
        }
    }

    private static void addNumbersTask(String path, HttpRequest req, Socket socket) throws IOException {
        String res = "";
        switch (path) {
            case "/addournumbers":
                res = addOurNumbers(req);
    ...
    private static void returnStaticFile(String root, String path, Socket socket) throws IOException, Exception {
        String html = getResourceFileContents(root + path);
        String httpResponse = "HTTP/1.1 200 OK\r\n\r\n" + html;
        socket.getOutputStream().write(httpResponse.getBytes("UTF-8"));
    }
